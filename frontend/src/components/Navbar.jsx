...
const Navbar = () => {
  const {navigate, user, setUser, axios, cart} = useContext(AppContext);

  const [isMenuOpen, setIsMenuOpen] = useState(false);


  const [isProfileOpen,
    setIsProfileOpen] = useState(false);
      ...
      <div className="flex items-center justify-between h-16">
        ...
        <div className="flex items-center">
          <Link to="/" className="text-2xl font-bold text-blue-600">
            <img src="https://github.com/Bobbyprogrammer/resturant-mern/blob/main/frontend/src/assets/logo.png?raw=true" alt="" className="w-32" />
          </Link>
        </div>
              ...
              <div className="relative">
                ...
                {isProfileOpen && (
                  <div
                    onMouseEnter={() => setIsProfileOpen(true)}
                    onMouseLeave={() => setIsProfileOpen(false)}
                    className="absolute right-0 mt-1 w-48 bg-white rounded-lg shadow-lg py-2 border border-gray-100"
                  >
                    <Link
                      to={"/my-bookings"}
